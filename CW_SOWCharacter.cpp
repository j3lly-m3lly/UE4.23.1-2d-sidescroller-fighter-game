...
ACW_SOWCharacter::ACW_SOWCharacter()
{
	...
	bPlayerDashing = false;
	bDashRight = false;
	DashAttackEnabled = false;
...
void ACW_SOWCharacter::UpdateAnimation()
{
...
void ACW_SOWCharacter::Tick(float DeltaSeconds)
{
...
void ACW_SOWCharacter::OnGetHit()
{
	UE_LOG(LogTemp, Warning, TEXT("2. oof!"));
}
...
void ACW_SOWCharacter::MoveRight(float Value)
{
	...
	if (!FMath::IsNearlyZero(Value) && !bPlayerDashing)
	{
		...
		//time for dash
		const float DashTime = .5f;

		const float CooldownTime = 2.f;
		...
		//Cooldown dash
		GetWorld()->GetTimerManager().SetTimer(THandle, TDel, CooldownTime, false);
...
void ACW_SOWCharacter::TouchStarted(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	// Jump on any touch
	Jump();
}

void ACW_SOWCharacter::TouchStopped(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	// Cease jumping once touch stopped
	StopJumping();
}

void ACW_SOWCharacter::UpdateCharacter()
{
